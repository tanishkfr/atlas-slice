...
export function PrincipleCard({ principle, bridge }: PrincipleCardProps) {
  ...
  return (
    <div className="relative border-l-2 border-accent py-6 pl-6 pr-4 sm:pl-8 sm:pr-6">
      ...
      <p className="font-mono text-[11px] font-medium uppercase tracking-[0.13em] text-accent">The rule</p>
      ...
      >
        ...
        {status === 'copied' ? 'Copied' : status === 'error' ? 'Copy blocked' : 'Copy rule'}
      </button>
      <p className="sr-only" aria-live="polite">
        {status === 'copied' ? 'Rule copied to the clipboard.' : status === 'error' ? 'Clipboard access was blocked. Select the rule text to copy it.' : ''}
      </p>
    </div>
  )
}
